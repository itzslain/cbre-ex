...
namespace CBRE.Providers.Texture
{
    public class MiscTexProvider : TextureProvider
    {
        ...
        public override IEnumerable<TexturePackage> CreatePackages(IEnumerable<TextureCategory> sourceRoots)
        {
            ...
            IEnumerable<TextureCategory> dirs = sourceRoots.Where(sr => Directory.Exists(sr.Path));

            foreach (TextureCategory dir in dirs)
            {
                TexturePackage tp = new TexturePackage(dir.Path, dir.CategoryName, this);

                IEnumerable<string> sprs = Directory.GetFiles(dir.Path, "*.*", SearchOption.AllDirectories).Where(s => s.EndsWith(".jpg") || s.EndsWith(".jpeg") || s.EndsWith(".png"));
                if (!sprs.Any()) continue;

                foreach (string spr in sprs)
                {
                    string rel = Path.GetFullPath(spr).Substring(dir.Path.Length).TrimStart('/', '\\').Replace('\\', '/');
