...
namespace CBRE.DataStructures.Geometric
{
    ...
    public class QuaternionF : ISerializable
    {
        ...
        {
            float lengthSq = (float)(Math.Pow(X, 2) + Math.Pow(Y, 2) + Math.Pow(Z, 2) + Math.Pow(W, 2));
        ...
        {
            QuaternionF q = W > 1 ? Normalise() : this;
            float angle = 2f * (float)Math.Acos(q.W);
            float denom = (float)Math.Sqrt(1 - q.W * q.W);
            CoordinateF coord = denom <= 0.0001 ? CoordinateF.UnitX : q.Vector / denom;
        ...
        public CoordinateF Rotate(CoordinateF coord)
        {
            ...
            QuaternionF q = new QuaternionF(coord.Normalise(), 0);
            QuaternionF temp = q * Conjugate();
        ...
        public static QuaternionF EulerAngles(CoordinateF angles)
        {
            ...
            angles = angles / 2;
            float sy = (float)Math.Sin(angles.Z);
            float sp = (float)Math.Sin(angles.Y);
            float sr = (float)Math.Sin(angles.X);
            float cy = (float)Math.Cos(angles.Z);
            float cp = (float)Math.Cos(angles.Y);
            float cr = (float)Math.Cos(angles.X);
        ...
        public static QuaternionF Lerp(QuaternionF start, QuaternionF end, float blend)
        {
            ...
            QuaternionF q1 = start.Clone();
            QuaternionF q2 = end.Clone();
            ...
            float blendA = 1 - blend;
            float blendB = blend;

            QuaternionF result = new QuaternionF(blendA * q1.Vector + blendB * q2.Vector, blendA * q1.W + blendB * q2.W);
