...
namespace CBRE.DataStructures.Geometric
{
    ...
    public class CoordinateF : ISerializable
    {
        ...
        {
            float xd = Math.Abs(X - test.X);
            float yd = Math.Abs(Y - test.Y);
            float zd = Math.Abs(Z - test.Z);
        ...
        {
            float xv = (Y * that.Z) - (Z * that.Y);
            float yv = (Z * that.X) - (X * that.Z);
            float zv = (X * that.Y) - (Y * that.X);
