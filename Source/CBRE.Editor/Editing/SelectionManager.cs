...
namespace CBRE.Editor.Editing
{
    public class SelectionManager
    {
        ...
        public void SwitchToFaceSelection()
        {
            ...
            foreach (Face face in SelectedFaces) face.IsSelected = false;
            SelectedFaces.Clear();
            ...
            SelectedFaces.UnionWith(GetSelectedObjects()
                                       ...
                                       .SelectMany(x =>
                                       {
                                           IEnumerable<Face> disps = x.Faces.Where(y => y is Displacement);
                                           return disps.Any() ? disps : x.Faces;
                                       }));

            foreach (Face face in SelectedFaces) face.IsSelected = true;

            foreach (MapObject obj in SelectedObjects) obj.IsSelected = false;
        ...
        public void Deselect(IEnumerable<MapObject> objs)
        {
            ...
            foreach (MapObject o in list) o.IsSelected = false;
        }
