...
namespace CBRE.Editor
{
    public static class Scheduler
    {
        ...
        private static void UpdateTimer()
        {
            ...
            {
                ScheduledCallback next = Callbacks.OrderBy(x => x.ScheduledTime).First();
                double delay = Math.Max(0, DateTime.Now.Subtract(next.ScheduledTime).TotalMilliseconds) + 100;
        ...
        private static void TimerCallback()
        {
            lock (Lock)
            {
                List<ScheduledCallback> expired = Callbacks.Where(x => x.ScheduledTime <= DateTime.Now).ToList();
