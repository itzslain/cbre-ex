...
namespace CBRE.DataStructures.Geometric
{
    public static class MathFunctions
    {
        ...
        {
            Vector4d source = new Vector4d(coordinate.DX, coordinate.DY, coordinate.DZ, 1);
            Vector4d imed = Vector4d.Transform(source, modelview);
            Vector4d vector = Vector4d.Transform(imed, projection);
            if (Math.Abs(vector.W - 0) < 0.00001) return null;
            Vector3d result = Vector3d.Divide(vector.Xyz, vector.W);
