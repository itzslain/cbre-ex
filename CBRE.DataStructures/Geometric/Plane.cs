...
namespace CBRE.DataStructures.Geometric
{
    ...
    public class Plane : ISerializable
    {
        ...
        public Coordinate GetIntersectionPoint(Line line, bool ignoreDirection = false, bool ignoreSegment = false)
        {
            ...
            Coordinate dir = line.End - line.Start;
            decimal denominator = -Normal.Dot(dir);
            decimal numerator = Normal.Dot(line.Start - Normal * DistanceFromOrigin);
            if (Math.Abs(denominator) < 0.00001m || (!ignoreDirection && denominator < 0)) return null;
            decimal u = numerator / denominator;
        ...
        public Coordinate GetClosestAxisToNormal()
        {
            ...
            Coordinate norm = Normal.Absolute();
        ...
        public static Coordinate Intersect(Plane p1, Plane p2, Plane p3)
        {
            ...
            Coordinate c1 = p2.Normal.Cross(p3.Normal);
            Coordinate c2 = p3.Normal.Cross(p1.Normal);
            Coordinate c3 = p1.Normal.Cross(p2.Normal);

            decimal denom = p1.Normal.Dot(c1);
            if (denom < 0.00001m) return null; // No intersection, planes must be parallel

            Coordinate numer = (-p1.D * c1) + (-p2.D * c2) + (-p3.D * c3);
            return numer / denom;
        }
