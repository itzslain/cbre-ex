...
namespace CBRE.DataStructures.MapObjects
{
    ...
    public class Face : ISerializable
    {
        ...
        {
            Face f = new Face(generator.GetNextFaceID())
            {
                Plane = Plane.Clone(),
                Colour = Colour,
            ...
            foreach (Vertex v in Vertices.Select(x => x.Clone()))
            {
                v.Parent = f;
                f.Vertices.Add(v);
            }
        ...
        public virtual void Paste(Face f)
        {
            ...
            Vertices.Clear();
            foreach (Vertex v in f.Vertices.Select(x => x.Clone()))
            {
                v.Parent = this;
                Vertices.Add(v);
            }
        ...
        {
            Coordinate direction = Plane.GetClosestAxisToNormal();
            Coordinate cp = Texture.UAxis.Cross(Texture.VAxis).Normalise();
        ...
        public void AlignTextureWithFace(Face face)
        {
            ...
            Coordinate refU = face.Texture.UAxis;
            Coordinate refV = face.Texture.VAxis;
            // Reference points in the texture plane to use for shifting later on
            Coordinate refX = face.Texture.UAxis * face.Texture.XShift * face.Texture.XScale;
            Coordinate refY = face.Texture.VAxis * face.Texture.YShift * face.Texture.YScale;
            ...
            Coordinate intersectionEdge = face.Plane.Normal.Cross(Plane.Normal);
            ...
            Coordinate intersect = Plane.Intersect(face.Plane, Plane, intersectionPlane);
            if (intersect != null)
            {
                Coordinate texNormal = face.Texture.GetNormal();
                ...
                Coordinate ptNormal = intersectionPlane.Project(texNormal).Normalise();
                Coordinate ppNormal = intersectionPlane.Project(Plane.Normal).Normalise();

                // Get the angle between the projected normals
                decimal dot = Math.Round(ptNormal.Dot(ppNormal), 4);
                decimal angle = DMath.Acos(dot); // A.B = cos(angle)

                // Rotate the texture axis by the angle around the intersection edge
                UnitRotate transform = new UnitRotate(angle, new Line(Coordinate.Zero, intersectionEdge));
        ...
        public void FitTextureToPointCloud(Cloud cloud, int tileX, int tileY)
        {
            ...
            List<decimal> xvals = cloud.GetExtents().Select(x => x.Dot(Texture.UAxis)).ToList();
            List<decimal> yvals = cloud.GetExtents().Select(x => x.Dot(Texture.VAxis)).ToList();

            decimal minU = xvals.Min();
            decimal minV = yvals.Min();
            decimal maxU = xvals.Max();
            decimal maxV = yvals.Max();
        ...
        public void AlignTextureWithPointCloud(Cloud cloud, BoxAlignMode mode)
        {
            ...
            List<decimal> xvals = cloud.GetExtents().Select(x => x.Dot(Texture.UAxis) / Texture.XScale).ToList();
            List<decimal> yvals = cloud.GetExtents().Select(x => x.Dot(Texture.VAxis) / Texture.YScale).ToList();
            ...
            switch (mode)
            {
                ...
                case BoxAlignMode.Center:
                    decimal avgU = (minU + maxU) / 2;
                    decimal avgV = (minV + maxV) / 2;
        ...
        {
            decimal rads = DMath.DegreesToRadians(Texture.Rotation - rotate);
            // Rotate around the texture normal
            Coordinate texNorm = Texture.VAxis.Cross(Texture.UAxis).Normalise();
            UnitRotate transform = new UnitRotate(rads, new Line(Coordinate.Zero, texNorm));
        ...
        public virtual void Transform(IUnitTransformation transform, TransformFlags flags)
        {
            ...
            if (flags.HasFlag(TransformFlags.TextureScalingLock) && Texture.Texture != null)
            {
                ...
                Coordinate origin = transform.Transform(Coordinate.Zero);
                Coordinate ua = transform.Transform(Texture.UAxis) - origin;
                Coordinate va = transform.Transform(Texture.VAxis) - origin;
            ...
            }
            {
                ...
                if (flags.HasFlag(TransformFlags.TextureLock) && Texture.Texture != null)
                {
                    ...
                    decimal scaled = (transform.Transform(Coordinate.One) - transform.Transform(Coordinate.Zero)).VectorMagnitude();
                    decimal original = (Coordinate.One - Coordinate.Zero).VectorMagnitude();
        ...
        {
            List<Coordinate> verts = Vertices.Select(x => x.Location).ToList();
        ...
        public PlaneClassification ClassifyAgainstPlane(Plane p)
        {
            ...
            foreach (int test in Vertices.Select(v => v.Location).Select(x => p.OnPlane(x)))
            {
        ...
        {
            Plane plane = new Plane(coordinates[0], coordinates[1], coordinates[2]);
            Coordinate intersect = plane.GetIntersectionPoint(line, ignoreDirection, ignoreSegment);
            ...
            for (int i = 0; i < coordinates.Count; i++)
            {
                ...
                Coordinate v1 = coordinates[i1] - intersect;
                Coordinate v2 = coordinates[i2] - intersect;

                double m1 = (double)v1.LengthSquared();
                double m2 = (double)v2.LengthSquared();
                double nom = m1 * m2;
                if (nom < 0.00001d)
                {
