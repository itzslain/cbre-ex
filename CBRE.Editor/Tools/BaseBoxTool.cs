    ...
    public abstract class BaseBoxTool : BaseTool
    {
        ...
        public class BoxState
        {
            ...
            public void FixBoxBounds()
            {
                ...
                Viewport2D vp = (Viewport2D)ActiveViewport;

                if (BoxStart.X > BoxEnd.X)
                {
                    decimal temp = BoxStart.X;
                    BoxStart.X = BoxEnd.X;
                    BoxEnd.X = temp;
                    Coordinate flat = vp.Flatten(Coordinate.UnitX);
                    if (flat.X == 1) SwapHandle("Left", "Right");
                    if (flat.Y == 1) SwapHandle("Top", "Bottom");
                }
                if (BoxStart.Y > BoxEnd.Y)
                {
                    decimal temp = BoxStart.Y;
                    BoxStart.Y = BoxEnd.Y;
                    BoxEnd.Y = temp;
                    Coordinate flat = vp.Flatten(Coordinate.UnitY);
                    if (flat.X == 1) SwapHandle("Left", "Right");
                    if (flat.Y == 1) SwapHandle("Top", "Bottom");
                }
                if (BoxStart.Z > BoxEnd.Z)
                {
                    decimal temp = BoxStart.Z;
                    BoxStart.Z = BoxEnd.Z;
                    BoxEnd.Z = temp;
                    Coordinate flat = vp.Flatten(Coordinate.UnitZ);
                    if (flat.X == 1) SwapHandle("Left", "Right");
                    if (flat.Y == 1) SwapHandle("Top", "Bottom");
                }
            }
        ...
        {
            Coordinate newStart = new Coordinate(Math.Min(start.X, end.X), Math.Min(start.Y, end.Y), Math.Min(start.Z, end.Z));
            Coordinate newEnd = new Coordinate(Math.Max(start.X, end.X), Math.Max(start.Y, end.Y), Math.Max(start.Z, end.Z));
        ...
        private static bool HandleHitTestLine(decimal hitX, decimal hitY, decimal test1X, decimal test1Y, decimal test2X, decimal test2Y, decimal hitbox)
        {
            ...
            decimal sx = Math.Min(test1X, test2X);
            decimal sy = Math.Min(test1Y, test2Y);
            decimal ex = Math.Max(test1X, test2X);
            decimal ey = Math.Max(test1Y, test2Y);
            bool hoz = test1Y == test2Y;
            return hoz
        ...
        {
            Coordinate start = new Coordinate(Math.Min(boxStart.X, boxEnd.X), Math.Min(boxStart.Y, boxEnd.Y), 0);
            Coordinate end = new Coordinate(Math.Max(boxStart.X, boxEnd.X), Math.Max(boxStart.Y, boxEnd.Y), 0);
        ...
        {
            Tuple<Coordinate, Coordinate> coords = GetResizedBoxCoordinates(viewport, e);
            Tuple<Coordinate, Coordinate> corrected = GetProperBoxCoordinates(coords.Item1, coords.Item2);
        ...
        protected virtual void MouseHoverWhenDrawn(Viewport2D viewport, ViewportEvent e)
        {
            ...
            ResizeHandle? handle = GetHandle(now, start, end, HandleWidth / viewport.Zoom);
            if (handle.HasValue)
            {
        ...
        {
            Coordinate origin = GetResizeOrigin(viewport);
        ...
        protected Tuple<Coordinate, Coordinate> GetResizedBoxCoordinates(Viewport2D viewport, ViewportEvent e)
        {
            ...
            Coordinate now = SnapIfNeeded(viewport.ScreenToWorld(e.X, viewport.Height - e.Y));
            ...
            bool proportional = KeyboardState.Ctrl && State.Action == BoxAction.Resizing && owidth != 0 && oheight != 0;
            ...
            switch (State.Handle)
            {
                ...
                case ResizeHandle.Center:
                    ...
                    Coordinate distance = GetResizeDistance(viewport, e);
        ...
        {
            Coordinate start = GetResizeOrigin(viewport);
            if (start == null) return;
            const int size = 6;
            double dist = (double)(size / viewport.Zoom);

            Coordinate origin = start + viewport.Flatten(State.BoxStart - State.PreTransformBoxStart);
        ...
        {
            double width = Math.Abs(start.DX - end.DX);
            double height = Math.Abs(start.DY - end.DY);
            double x1, y1, x2, y2;
            double handleWidth = width / 10d;
            double handleHeight = height / 10d;
            switch (State.Handle)
            {
                case ResizeHandle.TopLeft:
                    x1 = start.DX;
        ...
        {
            double[] box = GetRenderResizeBox(start, end);
            GL.Begin(PrimitiveType.Quads);
        ...
        protected void RenderBoxText(Viewport2D viewport, Coordinate boxStart, Coordinate boxEnd)
        {
            ...
            string widthText = (Math.Round(boxEnd.X - boxStart.X, 1)).ToString("#.##");
            string heightText = (Math.Round(boxEnd.Y - boxStart.Y, 1)).ToString("#.##");

            OpenTK.Graphics.TextExtents wid = _printer.Measure(widthText, _printerFont, new RectangleF(0, 0, viewport.Width, viewport.Height));
            OpenTK.Graphics.TextExtents hei = _printer.Measure(heightText, _printerFont, new RectangleF(0, 0, viewport.Width, viewport.Height));

            boxStart = viewport.WorldToScreen(boxStart);
            boxEnd = viewport.WorldToScreen(boxEnd);

            float cx = (float)(boxStart.X + (boxEnd.X - boxStart.X) / 2);
            float cy = (float)(boxStart.Y + (boxEnd.Y - boxStart.Y) / 2);

            RectangleF wrect = new RectangleF(cx - wid.BoundingBox.Width / 2, viewport.Height - (float)boxEnd.Y - _printerFont.Height - 18, wid.BoundingBox.Width * 1.2f, wid.BoundingBox.Height);
            RectangleF hrect = new RectangleF((float)boxEnd.X + 18, viewport.Height - cy - hei.BoundingBox.Height * 0.75f, hei.BoundingBox.Width * 1.2f, hei.BoundingBox.Height);
        ...
        protected bool GetSelectionBox(out Box boundingbox)
        {
            ...
            bool sameX = State.BoxStart.X == State.BoxEnd.X;
            bool sameY = State.BoxStart.Y == State.BoxEnd.Y;
            bool sameZ = State.BoxStart.Z == State.BoxEnd.Z;
            Coordinate start = State.BoxStart.Clone();
            Coordinate end = State.BoxEnd.Clone();
            bool invalid = false;
